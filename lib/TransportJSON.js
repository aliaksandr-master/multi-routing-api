  ...
  request (method, address, data, { disableParsing, ...options } = {}) {
    ...
    return window.fetch(address, fetchOptions)
      .then(
            ...
            .then(
              ({ response, result }) => {
                ...
                if (result instanceof Error) {
                  ...
                  responseInstance.status = this.defaultStatus || 0;
                }
        ...
        (err) => new Response({ ok: false, status: 0, result: null, error: err, rawResponse: null, options }) // in case request was canceled
      );
  }
}
