import { addSearchToUrl, isString, reduce, isNumber, isNaN } from './utils';
import {
  assertUniq
} from './assert';
...
const joinUrlPaths = (...paths) =>
  paths.reduce((result, path) => `${result.replace(/\/$/, '')}/${path.replace(/^\//, '')}`, '');
...
class RequestUrl {
  constructor (base, rawMask) {
    ...
    this.params = [];
    this.paramsByName = {};
    this.maskedUrlPath = rawMask.replace(/:([a-zA-Z0-9_$]+)/g, ($0, name) => {
      const mask = `<{{${name}}}>`;
