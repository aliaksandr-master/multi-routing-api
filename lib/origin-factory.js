...
import { assertFunction, assertInstanceOf, assertPlainObject, assertTrimmedNonEmptyString } from './assert';
...
const parseOpts = (opts = null, prevOpts = { params: {}, options: {} }) => {
  const {
    data = prevOpts.data,
...
export default ({ webResource = true, baseUrl = '' }, opts = null) => {
  ...
  const originOpts = parseOpts(opts);

  return {
    resource: (urlMack, opts = null) => {
      if (process.env.NODE_ENV !== 'production') {
      ...
      return {
        method: (method, opts = {}, argsToOpts = null, process = null) => {
          const methodOpts = parseOpts(opts, resourceOpts);

          if (argsToOpts !== null) {
            assertFunction('argsToProps', argsToOpts);
          } else {
            argsToOpts = (params = {}) => ({ params });
          }

          if (process !== null) {
            assertFunction('process', process);
          } else {
